...
class VectorSet:
    ...
    def from_list(
        ...
        # on vérifie si le fichier 'filelist.txt' contenant les chemins des données vecteur existe
        ...
        for path_vector_file in list_path_vector_files:
            ...
            if path_vector_file.endswith(".geojson"):
                ...
                except Exception as error_geojson_not_valid:
                    raise Exception(f"GeoJSON file {path_vector_file} is invalid: {error_geojson_not_valid}")
    ...
    def from_descriptor(cls, path_descriptor_file: str) -> "VectorSet":
        ...
        try:
            ...
            raise RuntimeError(
                f"command '{error_subprocess.cmd}' return with error (code {error_subprocess.returncode}): {error_subprocess.output}"
            )
        ...
        except JSONDecodeError as error_json_loading:
            raise FormatError("JSON", vectorset.__descriptor_object, error_json_loading)
...
class Vector:
    ...
    @classmethod
    def load_vector_data_content(cls, path: str) -> str:
        """charger le contenu des données vecteur à partir du chemin du fichier/objet vecteur
        ...
        if path.endswith(".geojson") or path.endswith(".gpkg") or path.endswith(".shp"):
            ...
            except Exception as error_json_loading:
                raise RuntimeError(f"Failed to load S3 object: {error_json_loading}")
        else:
            pass
        ...
        if self.__path_vector_file.endswith(".geojson"):
            ...
            try:
                self.vector_geojson = {
                    ...
                    "data": Vector.load_vector_data_content(self.__path_vector_file).__dict__,
                }
                # remplacer les éléments vides dans l'ancienne liste par une nouvelle liste sans élément vide
        ...
        if path.endswith(".geojson"):
            try:
                self.vector_geojson = {
                    ...
                    "data": Vector.load_vector_data_content(path).__dict__,
                }
                # remplacer les éléments vides dans l'ancienne liste par une nouvelle liste sans élément vide
                Vector.scrub_data_content_list(self.vector_geojson["tables"])
                # remplacer les éléments vides dans l'ancien dictionnaire par un nouveau dictionnaire sans élément vide
...
if __name__ == "__main__":
    ...
    ######################################################################
    # EXEMPLE 2 : OBTENIR DES SRS UNIQUES DES TABLES DES DONNEES VECTEUR #
    ######################################################################
