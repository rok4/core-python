...
class VectorSet:
    ...
    @classmethod
    def from_list(cls, path: str, tables: dict[str, list[dict[str,Union[str,int,tuple[float,float,float,float],list[str]]]]])-> "VectorSet":
        ...
        try:
            for index_path_vector_file in range(len(list_path_vector_files)):
                ...
                vectorset.__vectors.append([{"path" : list_path_vector_files[index_path_vector_file]},
                                            {"tables" : vectorset.__tables[index_path_vector_file]},                                         
                ])
        except OSError as error_not_valid_path:
            message_pattern: str = f"le chemin du fichier vecteur ou objet vecteur n'est pas valide {path},{error_not_valid_path}.\n"
    ...
    def from_descriptor(cls, path: str)-> "VectorSet":
        ...
        print ("[VectorSet/from_descriptor] vectorset.descriptor_file == "+str(vectorset.descriptor_file))
        print ("\n")
...
class Vector:
    ...
    def from_file(cls, path: str)-> "Vector":
        ...
        if vector.__path_vector_file.endswith('gpkg'):
            try:
                vector.__tables = {
                        tables[1][0]['name'] : Table(tables[1][0]['name'], tables[1][0]['srs'], tables[1][0]['count'], tables[1][0]['bbox'], tables[1][0]['attributes'], tables[1][0]['geometry_columns']).__dict__,
                        tables[1][1]['name'] : Table(tables[1][1]['name'], tables[1][1]['srs'], tables[1][1]['count'], tables[1][1]['bbox'], tables[1][1]['attributes'], tables[1][1]['geometry_columns']).__dict__,
                }
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
        elif vector.__path_vector_file.endswith('geojson'):
            try:
                vector.__tables = {
                        tables[0][0]['name'] : Table(tables[0][0]['name'], tables[0][0]['srs'], tables[0][0]['count'], tables[0][0]['bbox'], tables[0][0]['attributes'], tables[0][0]['geometry_columns']).__dict__,
                        }
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
            ...
            try:
                vector.__tables = {
                        tables[2][0]['name'] : Table(tables[2][0]['name'], tables[2][0]['srs'], tables[2][0]['count'], tables[2][0]['bbox'], tables[2][0]['attributes'], tables[2][0]['geometry_columns']).__dict__,
                        }
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
        else: 
        ...
        if vector.__path_vector_file.endswith('geojson') :
            ...
            try:
                ...
                raise Exception(f"Erreur à la lecture du contenu du geojson {error_reading_geojson.args}")
            ...
            try:
                ...
                print("[Vector/from_file] vector.__vector_geojson == "+str(vector.__vector_geojson))
                print("\n")
        ...
        # si on charge un fichier vecteur de type geopackage => fichier d'extension *.gpkg (ex : martinique.gpkg)
            ...
            try:
                vector.gpkg_data_content = subprocess.check_output('ogrinfo -json '+vector.__path_vector_file, shell=True,stderr=subprocess.STDOUT)
                
            except subprocess.CalledProcessError as error:
                raise RuntimeError("command '{}' return with error (code {}): {}".format(error.cmd, error.returncode, error.output))
        ...
        # si on charge un fichier vecteur de type shapefile => fichier d'extension *.shp (ex : TM_WORLD_BORDERS-0.3.shp)
            ...
            try:
                vector.shp_data_content = subprocess.check_output('ogrinfo -json '+vector.__path_vector_file, shell=True,stderr=subprocess.STDOUT)
            ...
            with open(vector.path_to_object_file, 'r') as geojson_file :
    ...
    def from_parameters(cls, path: str, table: dict[str, "Table"])-> "Vector":
        ...
        if vector.__path_vector_file.endswith('gpkg'):
            try:
                vector.__tables = {
                        table[0]["name"] : Table(table[0]['name'], table[0]['srs'], table[0]['count'], table[0]['bbox'], table[0]['attributes'], table[0]['geometry_columns']).__dict__,
                        table[1]["name"] : Table(table[1]['name'], table[1]['srs'], table[1]['count'], table[1]['bbox'], table[1]['attributes'], table[1]['geometry_columns']).__dict__,
                }
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
        else:
        ...
        if vector.__path_vector_file.endswith('geojson'):   
            try:
                ...
                print("[Vector/from_parameters] vector.__vector_geojson == "+str(vector.__vector_geojson))
                print("\n") 
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")       
            ...
            try:
                ...
                print("[Vector/from_parameters] vector.__vector_gpkg == "+str(vector.__vector_gpkg))
                print("\n")
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
            ...
            try:
                vector.__vector_object = {
                    ...
                    "tables": vector.__tables,
                    "data": str(vector.from_file(get_osgeo_path(vector.__path_vector_file)).object_s3_data_content),
                }

                print("[Vector/from_parameters] vector.__vector_object == "+str(vector.__vector_object))
                print("\n")
            except KeyError as error_key :
                raise MissingAttributeError(f"l'attribut {error_key.args} est manquant dans le vecteur de données.")
    ...
    def get_unique_srs_tables_list(srs: str)-> list[str]:
        ...
        if Vector._path_vector_file.endswith ('geojson'):
            print ("[Vector/get_unique_srs_tables_list] Vector._vector_geojson['tables']['crs']['properties']['name'] == "+json.dumps(Vector._vector_geojson["tables"]["crs"]["properties"]["name"]))
            list_uniq_srs_tables.append(Vector._vector_geojson["tables"]["crs"]["properties"]["name"])
        elif Vector._path_vector_file.endswith ('gpkg'):
            for index_layers in range(len(Vector._vector_gpkg["layers"])):
                for index_parameters in range(len(Vector._vector_gpkg["layers"][index_layers]["geometryFields"]["projjson"]["parameters"])):
                    list_uniq_srs_tables.append(Vector._vector_gpkg["layers"][index_layers]["geometryFields"]["projjson"]["parameters"][index_parameters]["id"]["code"])
                    print ("[Vector/get_unique_srs_tables_list] Vector.Vector._vector_gpkg['layers'][index_layers]['geometryFields']['projjson']['parameters'][index_parameters]['id']['code'] == "+json.dumps(Vector._vector_gpkg["layers"][index_layers]["geometryFields"]["projjson"]["parameters"][index_parameters]["id"]["code"]))
        elif Vector._path_vector_file.endswith ('shp'):
            for index_layers in range(len(Vector._vector_shp["layers"])):
                list_uniq_srs_tables.append(Vector._vector_shp["layers"][index_layers]["geometryFields"]["id"]["code"])
                print ("[Vector/get_unique_srs_tables_list] Vector._vector_shp['layers'][index_layers]['geometryFields']['id']['code'] == "+json.dumps(Vector._vector_shp["layers"][index_layers]["geometryFields"]["id"]["code"]))
        elif Vector._path_vector_file.startswith ('s3://'):
            print ("[Vector/get_unique_srs_tables_list] Vector._vector_object")
        else :
            pass

        print ("[Vector/get_unique_srs_tables_list] list_uniq_srs_tables == "+str(list_uniq_srs_tables))
        print ("\n")
...
class Table:
    ...
    def __init__(self, name: str, attributes: dict[str,dict[str,str]], count: int, srs:str, bbox: tuple[float, float, float, float], geometry_columns: list[str]) -> "Table":
...
if __name__ == '__main__' :
    ...
    pathtofilelisttxt = os.path.abspath(os.path.join(pathtoparentdir, 'tests/fixtures/filelist.txt'))
    vectorset = VectorSet()
