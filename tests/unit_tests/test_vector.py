...
def test_ok_csv2():
    try:
        vector_csv2 = VectorSet.from_list(
            "/tests/fixtures/vector2.csv",
...
def test_ok_shp():
    try:
        vector = VectorSet.from_list("/tests/fixtures/ARRONDISSEMENT.shp",
...
def test_ok_parameters():
    try:
        vector = VectorSet.from_parameters(
            "/tests/fixtures/ARRONDISSEMENT.shp",
            #"file://tests/fixtures/ARRONDISSEMENT.shp",
            (1, 2, 3, 4),
            [
                (
        ...
        assert str(vector.path) == "/tests/fixtures/ARRONDISSEMENT.shp"
    except Exception as exc:
        assert False, f"Vector creation raises an exception: {exc}"
