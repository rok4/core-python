...
from rok4.storage import disconnect_s3_clients, get_data_str, get_osgeo_path
...
def test_vector_from_parameters_ok_csv():
    ...
    try:
        ...
        assert str(vector_csv.path) == "file://tests/fixtures/vector.csv"
        assert (
...
def test_vector_from_parameters_ok_geojson():
    ...
    try:
        ...
        assert str(vector_geojson4.path) == "file://tests/fixtures/vector.geojson"
        assert (
...
def test_vector_from_parameters_ok_gpkg4():
    ...
    try:
        ...
        assert str(vector_gpkg4.path) == "file://tests/fixtures/vector.gpkg"
        assert (
...
def test_vectorset_from_list_ok_shp():
    ...
    try:
        vector_shp = VectorSet.from_list("file://tests/fixtures/ARRONDISSEMENT.shp")
        assert str(vector_shp.path) == "file://tests/fixtures/ARRONDISSEMENT.shp"
        assert (
...
def test_vector_from_parameters_ok_shp4():
    ...
    try:
        ...
        assert str(vector_shp4.path) == "file://tests/fixtures/ARRONDISSEMENT.shp"
        assert (
...
def test_vector_ok_parameters():
    ...
    try:
        ...
        assert str(vector5.path) == "file://tests/fixtures/vector2.csv"
        assert (
            str(vector5.tables)
            == "[('vector', 4, [('id', 'String'), ('x', 'String'), ('y', 'String')])]"
        )
