...
def test_wrong_content(mocked_copy):
    with pytest.raises(Exception) as exc:
        Vector.from_file("file:///vector.shp")
...
def test_get_data_valid(mock_checkoutput):
    ...
    mock_stdout.configure_mock(**{"stdout.decode.return_value": "returned non-zero exit status 1."})
