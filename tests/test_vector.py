...
def test_if_filelisttxt_exists()-> None:
    vectorset = VectorSet()
    pathtofilelist = "tests/fixtures/filelist.txt"
...
def test_geojson_file_path_is_ok_by_equals_assertion()->None:
    ...
    au jeu de données vecteur"""
    try:
        geojson_file_path = get_osgeo_path("tests/fixtures/states.geojson")
        assert geojson_file_path == "tests/fixtures/states.geojson"
    except Exception as exc:
        assert False, f" the path of vector geojson file from file {geojson_file_path} is not defined {exc}"
...
def test_gpkg_file_path_is_ok_by_equals_assertion()->None:
    ...
    au jeu de données vecteur"""
    try:
        gpkg_file_path = get_osgeo_path("tests/fixtures/martinique.gpkg")
        assert gpkg_file_path == "tests/fixtures/martinique.gpkg"
    except Exception as exc:
        assert False, f" the path of vector geopackage file from file {gpkg_file_path} is not defined {exc}"
...
def test_shp_file_path_is_ok_by_equals_assertion()->None:
    ...
    au jeu de données vecteur"""
    try:
        shp_file_path = get_osgeo_path("tests/fixtures/TM_WORLD_BORDERS-0.3.shp")
        assert shp_file_path == "tests/fixtures/TM_WORLD_BORDERS-0.3.shp"
    except Exception as exc:
        assert False, f" the path of vector shapefile from file {shp_file_path} is not defined {exc}"
...
def test_by_mocking_open_function_with_a_not_valid_vector_object(mocked_file_s3_vector_object_not_valid)->None:
    vectorset = VectorSet()
...
@patch.object(builtins, "open", new_callable=mock_open, read_data="tests/fixtures/states.geojson")
@patch.object(builtins, "open", new_callable=mock_open, read_data="tests/fixtures/TM_WORLD_BORDERS-0.3.shp")
@patch.object(builtins, "open", new_callable=mock_open, read_data="tests/fixtures/martinique.gpkg")
def test_reading_vector_files_by_mocking_open_function_all_parameters_ok(mocked_file_open_geojson, mocked_file_open_shp, mocked_file_open_gpkg)->None:
    ...
    assert isinstance(output_shp, str)
    assert isinstance(output_gpkg, str)
    # Vérifier que le contenu lu est égal à ["tests/fixtures/states.geojson","tests/fixtures/TM_WORLD_BORDERS-0.3.shp","tests/fixtures/martinique.gpkg"]
    expected_output = ["tests/fixtures/states.geojson", "tests/fixtures/TM_WORLD_BORDERS-0.3.shp", "tests/fixtures/martinique.gpkg"]
...
@patch("builtins.open", new_callable=mock_open, read_data="tests/fixtures/filelist_not_valid.txt")
def test_if_listtxtpath_is_not_ok_by_mocking_open_function_with_a_not_valid_filelist(mocked_file)-> None:
    vectorset = VectorSet()
    with pytest.raises(Exception):
...
def test_bad_json()->None:
    vectorset = VectorSet()
    path_to_descriptor_file = "tests/fixtures/vectorset.json"
    bad_json = {
                "path": "my_path",
                "table": "my_table",
                "data": [
...
def test_missing_vector_keys()->None:
    vectorset = VectorSet()
    descriptor_file_with_missing_keys = [{'path': 'file://./data/martinique.gpkg',}]
    REQUIRED_VECTOR_KEYS = ["path", "tables"]
    with patch("rok4.vector.VectorSet.from_descriptor", return_value="[{'path': 'file://./data/martinique.gpkg',}]") as mocked_missing_vector_keys:
...
def test_if_descriptor_contains_valid_json_data(mocked_check_output)->None:
    vectorset = VectorSet()
    path_to_descriptor_file = "tests/fixtures/vectorset.json"
    # Appelons la méthode qui déclenche le bloc "try/except"
    vectorset.from_descriptor(path_to_descriptor_file)
    mocked_check_output.call_count == 1

@patch('subprocess.check_output')
def test_if_descriptor_contains_data_content_error(mocked_check_output)->None:
    vectorset = VectorSet()
    path_to_descriptor_file = "tests/fixtures/vectorset.json"
    mocked_check_output.side_effect = subprocess.CalledProcessError(
        1, "ogrinfo -json tests/fixtures/vectorset_with_error.json", b"error output"
    )
...
@patch("json.load", return_value = dict({"the_data": "This is fake data"}))
def test_descriptor_is_not_valid_with_a_fake_file_path(mocked_json_load)->None:
    vectorset = VectorSet()
    path_to_fake_descriptor = "/non_exists/fake_descriptor.json"
    mocked_json_load.side_effect = json.load(JSONDecodeError("JSON", path_to_fake_descriptor,1))  
    with pytest.raises(Exception):
        vectorset.from_descriptor(path_to_fake_descriptor)
    mocked_json_load.assert_called_once()

@patch.object(VectorSet, "get_uniq_srs_tables_list", return_value=["EPSG:4326", "EPSG:3857", "EPSG:4559"])
...
def test_scrubbed_list_ok_by_returning_good_data_content()->None:
    data_content = {'path': "my_path", 'tables': ["table1", "table2"], "data": "my_data_content"}
    with patch.object(Vector,"scrub_data_content_dict", return_value=data_content) as mocked_scrub:
        output_data_content = Vector.scrub_data_content_dict(data_content)
        assert isinstance(output_data_content, dict)
        assert output_data_content == data_content
    assert mocked_scrub.call_count == 1
...
def test_vector_files_all_are_ok()->None:
    ...
    pathtofilegeojson = "tests/fixtures/states.geojson"
    with patch.object(Path, "exists", return_value=True) as mocked_path_exists:
        with pytest.raises(Exception):
    ...
    vectorset = VectorSet()
    pathtofilegeopackage = "tests/fixtures/martinique.gpkg"
    with patch.object(Path, "exists", return_value=True) as mocked_path_exists:
        with pytest.raises(Exception):
    ...
    vectorset = VectorSet()
    pathtofileshapefile = "tests/fixtures/TM_WORLD_BORDERS-0.3.shp"
    with patch.object(Path, "exists", return_value=True) as mocked_path_exists:
        with pytest.raises(Exception):
...
def test_s3_vector_objects_all_are_ok()->None:
    """tester que les objets vecteurs de type objet s3 existent bien,
    sont bien des fichiers et sont bien lisibles
    """
    # on teste pour l'objet vecteur de type objet S3 
    # si l'url existe bien
    vectorset = VectorSet()
    url_to_s3_vector_object = "s3://my_vector_object.json"
    with patch.object(Path, "exists", return_value=True) as mocked_path_exists:
        with pytest.raises(Exception):
            vectorset.from_list(url_to_s3_vector_object)
    mocked_path_exists.assert_called()

    # si c'est bien un objet de type S3
    url_to_vector_object = "s3://my_vector_object.json"
    with pytest.raises(Exception):
        mocked_is_s3_object = Mock()
        mocked_is_s3_object.startswith.return_value = True 
        mocked_is_s3_object.startswith(url_to_vector_object)
        result = vectorset.from_list(mocked_is_s3_object)
        assert result == True
    mocked_is_s3_object.assert_not_called()

    # si l'objet vecteur de type S3 est bien lisible
    vectorset = VectorSet()
    s3_object_not_readable = "s3://not_readable"
    with patch.object(request,"urlopen", return_value=True) as mocked_is_readable:
        with pytest.raises(Exception):
            vectorset.from_list(s3_object_not_readable)
    mocked_is_readable.assert_not_called()

def test_vector_files_raises_exception_with_faked_vector_files()->None:
    """tester que les fichiers vecteurs (geojson, geopackage et shapefile) n'existent pas,
    ne sont pas des fichiers et ne sont pas lisibles
    """
    # on teste pour le fichier *.geojson
    # si le fichier geojson n'existe pas   
    vectorset = VectorSet()
    pathtofilegeojson = "/non_exists/fake.geojson"
    with patch.object(Path, "exists", return_value=False) as mocked_path_exists:
        with pytest.raises(Exception):
            vectorset.from_list(pathtofilegeojson)
    mocked_path_exists.assert_called()
    
    # si le fichier geojson n'est pas un fichier
    vectorset = VectorSet()
    with patch.object(Path, "is_file", return_value=False) as mocked_is_file:
        with pytest.raises(Exception):
            vectorset.from_list(pathtofilegeojson)
    mocked_is_file.assert_called()
    
    # si le fichier geojson n'est pas lisible 
    vectorset = VectorSet()
    file_geojson_not_readable = "fake_not_readable.geojson"
    with patch("os.access", return_value=False) as mocked_is_readable:
        with pytest.raises(Exception):
            vectorset.from_list(file_geojson_not_readable)
    mocked_is_readable.assert_called()
    assert not os.access(pathtofilegeojson, os.R_OK)

    # on teste pour le fichier *.geopackage
    # si le fichier geopackage n'existe pas
    vectorset = VectorSet()
    pathtofilegeopackage = "/non_exists/fake.gpkg"
    with patch.object(Path, "exists", return_value=False) as mocked_path_exists:
        with pytest.raises(Exception):
            vectorset.from_list(pathtofilegeopackage)
    mocked_path_exists.assert_called()
    
    # si le fichier geopackage n'est pas un fichier
    vectorset = VectorSet()
    with patch.object(Path, "is_file", return_value=False) as mocked_is_file:
        with pytest.raises(Exception):
            vectorset.from_list(pathtofilegeopackage)
    ...
    # si le fichier geopackage n'est pas lisible
    vectorset = VectorSet()
    file_gpkg_not_readable = "fake_not_readable.gpkg"
    with patch("os.access", return_value=False) as mocked_is_readable:
        with pytest.raises(Exception):
            vectorset.from_list(file_gpkg_not_readable)
    mocked_is_readable.assert_called()
    assert not os.access(pathtofilegeopackage, os.R_OK)

    # on teste pour le fichier *.shapefile
    # si le fichier shapefile n'existe pas
    vectorset = VectorSet()
    pathtofileshapefile = "/non_exists/fake.shp"
    with patch.object(Path, "exists", return_value=False) as mocked_path_exists:
        with pytest.raises(Exception):
            vectorset.from_list(pathtofileshapefile)
    mocked_path_exists.assert_called()
    ...
    # si le fichier shapefile n'est pas lisible
    vectorset = VectorSet()
    file_shp_not_readable = "fake_not_readable.shp"
    with patch("os.access", return_value=False) as mocked_is_readable:
        with pytest.raises(Exception):
            vectorset.from_list(file_shp_not_readable)
    mocked_is_readable.assert_called()
    assert not os.access(pathtofileshapefile, os.R_OK)

def test_s3_vector_objects_raises_exception_with_faked_vector_objects()->None:
    """tester que les objets vecteurs feints n'existent pas,
    ne sont pas des fichiers et ne sont pas lisibles
    """
    # on teste pour l'objet vecteur de type objet S3 
    # si l'url n'existe pas
    vectorset = VectorSet()
    url_to_s3_vector_object = "s3://my_vector_object_non_exists.json"
    with patch.object(Path, "exists", return_value=False) as mocked_path_exists:
        with pytest.raises(Exception):
            vectorset.from_list(url_to_s3_vector_object)
    mocked_path_exists.assert_called()

    # si ce n'est pas un objet veceur de type objet S3
    # get_osgeo_path("s3://bucket@b/to/object.ext")
    url_to_vector_object = "my_vector_object_non_exists.json" 
    with patch('rok4.vector.Vector.from_file', 'get_osgeo_path') as mocked_is_s3_object:
        with pytest.raises(Exception):
            result = vectorset.from_list(url_to_vector_object)
            assert result == False
    mocked_is_s3_object.startswith("s3://") == False

    # si l'objet vecteur de type S3 n'est pas lisible
    vectorset = VectorSet()
    s3_object_not_readable = "s3://fake_not_readable.json"
    with patch.object(request, "urlopen", return_value=False) as mocked_is_readable:
        with pytest.raises(Exception):
            vectorset.from_list(s3_object_not_readable)
    mocked_is_readable.assert_not_called()
...
@patch.object(Vector, "get_uniq_srs_tables_list", return_value=["EPSG:4326"])
...
def test_vector_get_uniq_srs_tables_list_is_ok_for_all_vector_files()->None:
    ...
    vector_geojson = Vector.from_parameters("tests/fixtures/states.geojson", tables[2])
    srs_uniques_geojson = vector_geojson.get_uniq_srs_tables_list
    assert srs_uniques_geojson == ['EPSG:3857']

    vector_gpkg = Vector.from_parameters("tests/fixtures/martinique.gpkg", tables[0])
    srs_uniques_gpkg = vector_gpkg.get_uniq_srs_tables_list
    assert srs_uniques_gpkg == ['EPSG:4559']

    vector_shp = Vector.from_parameters("tests/fixtures/TM_WORLD_BORDERS-0.3.shp", tables[1])
...
def test_vectorset_get_uniq_srs_tables_list_is_ok()->None:
    vectorset_desc = VectorSet.from_descriptor("tests/fixtures/vectorset.json")
    result = vectorset_desc.get_uniq_srs_tables_list
    assert set(result) == set(['EPSG:4559', 'EPSG:4326', 'EPSG:3857'])
...
@patch('subprocess.check_output',return_value=b'{"type": "FeatureCollection", "features": []}')
def test_geojson_data_content_success(mocked_check_output)->None:
    vector = Vector()
    vector._Vector__path_vector_file = "tests/fixtures/states.geojson"
    # Appelons la méthode qui déclenche le bloc "try/except"
    vector.from_file("tests/fixtures/states.geojson")
    mocked_check_output.assert_called_once_with(
        'ogrinfo -json tests/fixtures/states.geojson', shell=True, stderr=subprocess.STDOUT
    )

@patch('subprocess.check_output')
def test_geojson_data_content_error(mocked_check_output)->None:
    mocked_check_output.side_effect = subprocess.CalledProcessError(
        1, "ogrinfo -json tests/fixtures/states.geojson", b"error output"
    )
    vector = Vector()
    vector._Vector__path_vector_file = "tests/fixtures/states.geojson"
    with pytest.raises(RuntimeError) as excinfo:
        vector.from_file("tests/fixtures/states.geojson")
    assert "command" in str(excinfo.value)
    mocked_check_output.assert_called()
...
def test_if_table_properties_are_all_ok()->None:
    ...
    table = Table(name="test_table", geometry_columns=["id", "value"], bbox=(180.0,2.0,3.0,4.0), srs="EPSG:2154", attributes={
        "id": "String",
        "STATE_ABBR": "String",
        "STATE_NAME": "String",
        "AREA_LAND": "Real",
    ...
    assert table.attributes == {"id": "String",
        "STATE_ABBR": "String",
        "STATE_NAME": "String",
        "AREA_LAND": "Real",
        "AREA_WATER": "Real",
