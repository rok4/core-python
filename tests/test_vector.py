...
def test_ok_geojson():
    try:
        vector = VectorSet.from_list("file://tests/fixtures/vector.geojson")
        assert (
            str(vector.layers)
...
def test_ok_gpkg():
    try:
        vector = VectorSet.from_list("file://tests/fixtures/vector.gpkg")
        assert (
            str(vector.layers)
...
def test_ok_shp():
    try:
        vector = VectorSet.from_list("file://tests/fixtures/ARRONDISSEMENT.shp")
        assert (
            str(vector.layers)
